...
import haitong.yao.byrclient.constant.ConstantData;
...
public class NetUtil {
    ...
    private static HttpURLConnection getHttpGetConnection(String url,
        ...
        try {
            ...
            connection.setRequestProperty("Host", ConstantData.BYR_HOST);
    ...
    public static Object login(Context context, String username, String password) {
        User user = null;
        String url = ConstantData.BYR_API + ConstantData.API_USER_LOGIN
                + ENCODE_TYPE_JSON + "?appkey=" + APP_KEY;
    ...
    public static Object queryUser(Context context, String id) {
        User user = null;
        String url = ConstantData.BYR_API + ConstantData.API_USER_QUERY + id
                + ENCODE_TYPE_JSON + "?appkey=" + APP_KEY;
        HttpURLConnection connection = getHttpGetConnection(url, GET_METHOD,
                false);
        if (null == connection) {
    ...
    public static Object getArticles(Context context, String type) {
        ...
        if (type.equals("topten")) {
            url = ConstantData.BYR_API + ConstantData.API_WIDGET_TOPTEN
                    + ENCODE_TYPE_JSON + "?appkey=" + APP_KEY;
        } else if (type.equals("hot")) {
            url = ConstantData.BYR_API + ConstantData.API_WIDGET_FOCUS
                    + ENCODE_TYPE_JSON + "?appkey=" + APP_KEY;
        } else if (type.equals("recommended")) {
            url = ConstantData.BYR_API + ConstantData.API_WIDGET_RECOMMEND
                    + ENCODE_TYPE_JSON + "?appkey=" + APP_KEY;
        }
        if (null == url) {
            return null;
    ...
    public static Object getFavourites(Context context, int level) {
        ...
        String url = ConstantData.BYR_API + ConstantData.API_FAVOURITE + level
                + ENCODE_TYPE_JSON + "?appkey=" + APP_KEY;
        HttpURLConnection connection = getHttpGetConnection(url, GET_METHOD,
                false);
        if (null == connection) {
    ...
    public static Object getMails(Context context, String type) {
        ...
        if (type.equals("inbox") || type.equals("outbox")
                || type.equals("deleted")) {
            url = ConstantData.BYR_API + ConstantData.API_MAILBOX + type
                    + ENCODE_TYPE_JSON + "?appkey=" + APP_KEY;
        }
        if (null == url) {
            return null;
